        ...
        <asp:UpdatePanel ID="udpnlChatRoom" runat="server" UpdateMode="Conditional" >
            <ContentTemplate>
                ...
                <asp:Repeater ID="rptDialog" runat="server" onitemcommand="rptDialog_ItemCommand" onitemdatabound="rptDialog_ItemDataBound">
                    ...
                    <ItemTemplate>
                        <asp:HiddenField ID="hfdPK_lTinNhan" runat="server" Value='<%#Eval("PK_lMessage")%>' />
                       <%-- <span style="color: <%=sColor%>">--%>
                        <asp:HiddenField ID="hfFK_sNguoiGui" runat="server" Value='<%#Eval("FK_sUsername")%>' />
                            <asp:Label ID="lblFK_sNguoiGui" runat="server" Text='<%#Eval("FK_sUsername")%>' Font-Bold="true"></asp:Label>
                        <%--</span>--%>
                        <span style="font-size:10px">
                            (<asp:Label ID="lbltNgayGui" runat="server" Text='<%#Eval("tDateSent")%>'></asp:Label>)
                        </span>: 
                        <asp:Label ID="lblsNoidung" runat="server" Text='<%#Eval("sContent")%>'></asp:Label>
                        <div class="divshow">
