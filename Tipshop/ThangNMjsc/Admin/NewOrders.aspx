<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="NewOrders.aspx.cs" Inherits="ThangNMjsc.Admin.NewOrders" MasterPageFile="~/MasterPage/PublicAdmin.Master"%>
...
<asp:Content ID="Content" ContentPlaceHolderID="cphMainContent_Admin" runat="server">
    ...
    <div>
        <center><asp:Label ID="Label8" runat="server" Text="Tất cả các Hóa đơn chưa được xử lý" CssClass="tieude"></asp:Label><br /><br />
        ...
        <cc1:gridviewext id="grvListOrder" runat="server" cssclass="mGrid" autogeneratecheckboxcolumn="True"
            ...
            <columns>
                ...
                <asp:BoundField DataField="Pay_DateOfStart" HeaderText="Ngày đặt mua" DataFormatString="{0:dd/MM/yyyy HH:mm}">
                    <ItemStyle CssClass="GridItemDate" />
                </asp:BoundField>
                <asp:BoundField DataField="Pay_DateOfFinish" HeaderText="Ngày giao hàng" DataFormatString="{0:dd/MM/yyyy HH:mm}">
                    <ItemStyle CssClass="GridItemDate" />
                </asp:BoundField>
                <asp:BoundField DataField="Pay_Status" HeaderText="Trạng thái">
                    <ItemStyle CssClass="GridItemText" />
                </asp:BoundField>
            </columns>
        </cc1:gridviewext>
        <center>
            <asp:Button ID="btnExcute" runat="server" Text="Xử lý đơn hàng" OnClick="btnExcute_Click" OnClientClick="return confirm('Bạn có muốn xử lý Hóa đơn đã chọn ko?');" />
            <asp:Button ID="btnDelete" runat="server" Text="Xóa" OnClick="btnDelete_Click" OnClientClick="return confirm('Bạn có muốn xóa Hóa đơn đã chọn ko?');" />
        </center>
    </div>
</asp:Content>